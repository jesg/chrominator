 ...
 *
 * @example
 * node = await driver.until_not(ExpectedConditions.is_node_present({selector: 'button[value="Search"]'}))
 *
 * @param {function} condition - The condition to wait for.
 */
...
Driver.prototype.waitForNodeNotPresent = function (selector) {
  return this.wait.until_not(ExpectedConditions.is_node_present({selector: selector}))
}
