...
Driver.prototype._buildArgs = function(rawArgs) {
    ...
    for (let i = 0; i < rawArgs.length; i++) {
        const arg = rawArgs[i];
        if (typeof arg.toRemoteObject === 'function') {
          promises.push(arg.toRemoteObject().then((result) => {
              const ii = i;
              args[ii] = {objectId: result.object.objectId};
          }));
        } else if (typeof arg.objectId === 'string') {
            args[i] = {objectId: arg.objectId};
        } else {
          args[i] = {value: arg}
        }
    }
    return Promise.all(promises).then(() => { return args; });
};
